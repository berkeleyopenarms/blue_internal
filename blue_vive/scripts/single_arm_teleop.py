...
class GripperClient(object):
    def __init__(self):
        side = sys.argv[1]
        ns = '/' + side + '_arm/blue_controllers/' + 'gripper_controller/'
        ...
        if not self._client.wait_for_server(rospy.Duration(10.0)):
            ...
            sys.exit(1)
        self.clear()
        rospy.Subscriber("/" + side + "_trigger", Float32, self.goal_callback, queue_size=1)
...
def main():
    global command_publisher
    side = sys.argv[1]
    rospy.init_node(side + "_gripper_action_client")
    command_publisher = rospy.Publisher('/' + side + '_arm/blue_controllers/cartesian_pose_controller/command', PoseStamped, queue_size=1)
