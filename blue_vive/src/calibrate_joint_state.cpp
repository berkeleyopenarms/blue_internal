...
public:
  ...
  CalibrateJointState(std::vector<KDL::Chain> chains) { 
    ...
    if (!node.getParam("blue_hardware/calibration_initial_angles", calibration_initial_angles)) {
  ...
  void update()
  {
    ...
    for(int calibration_index = 0; calibration_index < fk_poses_desired.size(); calibration_index++) {
      ...
      for(int iteration = 0; iteration < num_iterations; iteration++)
      {
        ...
        counter++;
        // change for 6 blue_hardware version
...
int main(int argc, char** argv)
{
  ...
  //if (!node.getParam("/blue_hardware/endlink_tracker",  end_tracker_link)) {
