  ...
  void jointCallback(const sensor_msgs::JointState msg)
  {
    ...
    for (int j = 0; j < 40; j++)
    {
      ...
      if (posture_control) {
        ...
        for (int i = 0; i < 6; i++) { for (int j = 0; j < nj; j++) { jacobian_eig(i, j) = jacobian(i, j);
          }
        }
