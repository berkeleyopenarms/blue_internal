...
#import vive_tracking as vt
sys.path.append('../../koko_ws/src/blue_internal/blue_vive_tracking/scripts')
...
for i in range(num_cycles): #iterate over the number of cycles
	...
	for j in range(len(location_list)): #iterate over the shuffled pose list	
		kk.set_joint_positions(np.array(location_list[j][1])) # tell the robot to go to a set of joint angles
		print('position set')
