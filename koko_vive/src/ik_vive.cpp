...
public:
  ...
  SubscribeAndPublish(KDL::Chain chain): fksolver(chain), jacSolver(chain)
  {
    ...
    subForearmTracker = n_.subscribe("/forearm_tracker_pose", 2, &SubscribeAndPublish::forearmTrackerCallback, this);
  ...
  void update()
  {
    ...
    if (!(base_tracker_read && forearm_tracker_read)){
      ROS_INFO("not received both base and forearm trackers");
      return;
    }
    ...
    ee_pos_desired(0, 0) = forearm_tracker_pose_local.pose.position.x;
    ee_pos_desired(1, 0) = forearm_tracker_pose_local.pose.position.y;
    ee_pos_desired(2, 0) = forearm_tracker_pose_local.pose.position.z;
    ...
    KDL::Rotation desired_rotation = KDL::Rotation::Quaternion(forearm_tracker_pose_local.pose.orientation.x,
                                                               forearm_tracker_pose_local.pose.orientation.y,
                                                               forearm_tracker_pose_local.pose.orientation.z,
                                                               forearm_tracker_pose_local.pose.orientation.w
                                                               );
